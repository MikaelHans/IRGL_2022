...
public class airplane : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && GameStart == false && !checkIfPlayerHasSpawned() /*&& !PhotonNetwork.IsMasterClient*/)
        {
        ...
        {
            if (checkIfPlayerHasSpawned() == false && !PhotonNetwork.IsMasterClient)
            {
                has_dropped = true;
                object[] data = new object[1];
                data[0] = cloud.teamID;
